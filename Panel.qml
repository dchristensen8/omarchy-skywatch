...
Panel {
  id: root
  moduleName: "dchristensen8.skywatch"
  ipcTarget: "dchristensen8.skywatch"
  manageIpc: false
  ...
  KeyboardPanel {
    ...
    PanelKeyCatcher {
      ...
      // ---- Interactive Omastorm radar below the forecast. Loaded lazily so a
      // missing Omastorm install degrades to a hint instead of breaking the
      // weather panel (see README: radar requires the Omastorm plugin).
      Rectangle {
        visible: true
        width: parent.width
        height: Style.spacing.hairline
      ...
      Loader {
        id: radarLoader
        width: parent.width
        source: Qt.resolvedUrl("RadarSection.qml")
        onLoaded: {
          item.expandRequested.connect(root.expandRadar)
